...
function ENT:TraceHUDPaint()
	if self.PHealth then
		...
		if self.IsMine then
			draw.SimpleText("Deployable Spawnpoint (Bound)", "teamplay", x, y, COLOR_CYAN, TEXT_ALIGN_CENTER)
		else
			draw.SimpleText("Deployable Spawnpoint (Not bound)", "teamplay", x, y, COLOR_CYAN, TEXT_ALIGN_CENTER)
		end
