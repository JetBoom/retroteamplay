...
function ENT:Initialize()
	...
	for _, ent in pairs(ents.FindInSphere(self:GetPos(), 90)) do
		if ent:GetClass() == self:GetClass() and ent ~= self then
			if ent:GetOwner():Team() == self:GetOwner():Team() then
				ent:Remove()
			end
		end
	end
...
function ENT:Think()
	...
	for _, ent in pairs(ents.FindInSphere(pos, 90)) do
		if (ent:IsPlayer() or firedamage and ent.ScriptVehicle) and TrueVisible(pos, ent:NearestPoint(pos)) then
			if firedamage then
				ent:TakeSpecialDamage(2, DMGTYPE_FIRE, owner, DUMMY_FIRECLOUD)
				if 0 < ent:Health() then
					local firecaster = self.FireCaster
					if not (firecaster and firecaster:IsPlayer()) then firecaster = owner end
					ent:TakeSpecialDamage(2, DMGTYPE_FIRE, firecaster, DUMMY_FIRECLOUD)
				end
			else
				ent:TakeSpecialDamage(1, DMGTYPE_POISON, owner, self)
			end
		end
	end
...
function ENT:ProcessDamage(attacker, inflictor, dmginfo)
	if dmginfo:GetDamageType() == DMGTYPE_FIRE and dmginfo:GetDamage() >= 40 and self:GetSkin() ~= 1 then
		self:SetSkin(1)
