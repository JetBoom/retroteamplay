...
function ENT:PilotEnter(pl)
	...
	if passenger:IsValid() and passenger:GetTeamID() ~= teamid then
...
function ENT:Think()
	...
	if driver:IsValid() and driver:KeyDown(IN_RELOAD) and CurTime() > self.NextHonk then
		self:EmitSound("npc/combine_gunship/see_enemy.wav", 160, math.Rand(88, 112))
		self.NextHonk = CurTime() + 3
	end
...
function ENT:PhysicsCollide(data, phys, frompart)
	...
	if hitentclass ~= "prop_physics_multiplayer" and ((not hitent:IsValid() or string.find(hitentclass, "prop_", 1, true) or hitentclass == "obelisk" or hitent.ScriptVehicle or hitent:GetVehicleParent():IsValid()) and not (hitent.ScriptVehicle and hitent:GetTeamID() == self:GetTeamID() or hitent:GetVehicleParent():IsValid() and hitent:GetVehicleParent():Team() == self:GetTeamID()) and 200 < data.Speed) then
