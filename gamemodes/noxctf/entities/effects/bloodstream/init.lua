...
local function CollideCallback(oldparticle, hitpos, hitnormal)
	...
	local emitter = ParticleEmitter(pos)
		for i=1, math.random(-4, 4) do
			...
			particle:SetVelocity(VectorRand():GetNormal() * math.random(75, 150) + nhitnormal)
...
function EFFECT:Init(data)
	...
	local emitter = ParticleEmitter(Pos)
		for i=1, data:GetMagnitude() do
			...
			particle:SetLighting(true)
			particle:SetVelocity(VectorRand():GetNormal() * math.Rand(force * 0.4, force * 0.7) + Vector(0,0,force * 0.5) + force * math.Rand(0.8, 1.2) * dir)
