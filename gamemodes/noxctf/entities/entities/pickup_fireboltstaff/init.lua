...
function ENT:Use(pl)
	...
	if classname == "Wizard" or classname == "Conjurer" then
		if pl:HasWeapon("staff_firebolt") then
		...
		if #pl:GetWeapons() > 0 then
			...
			if wep.Droppable then
				...
				if ent:IsValid() then
					...
					ent.Dropped = true
					ent:Spawn()
					if wep.ChargeAmmo then
						ent.Mana = pl:GetAmmoCount(wep.ChargeAmmo)
					end
		...
		pl:Give("staff_firebolt")
		if self.Mana then
			pl:GetWeapon("staff_firebolt").Mana = self.Mana
		end
