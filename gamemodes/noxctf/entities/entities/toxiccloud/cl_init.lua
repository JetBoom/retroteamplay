	...
	if EFFECT_QUALITY < 2 then
		...
		if self:GetSkin() == 1 then
			for i=1, math.random(4, 9) do
				local particle = emitter:Add("effects/fire_cloud"..math.random(1,2), pos + Vector(math.Rand(-128, 128), math.Rand(-128, 128), 1):GetNormal() * math.Rand(16, 128))
			...
			for i=1, math.random(4, 9) do
				local particle = emitter:Add("particles/smokey", pos + Vector(math.Rand(-128, 128), math.Rand(-128, 128), 1):GetNormal() * math.Rand(16, 128))
				particle:SetDieTime(0.9)
				particle:SetStartAlpha(220)
				particle:SetEndAlpha(0)
