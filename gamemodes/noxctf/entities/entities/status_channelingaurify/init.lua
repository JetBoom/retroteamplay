...
function ENT:StatusShouldRemove(owner)
	return owner:GetVelocity():Length() > 20 or owner:GetStatus("manastun") or owner:GetStatus("aurify") or owner:GetStatus("frozen") or owner:GetMana() < self.ManaPerSecond * self.TickTime or not self:IsValidTarget(self:GetTarget())
end
...
function ENT:StatusOnRemove(owner)
	...
	RestoreSpeed(target)
	if CurTime() >= self:GetDieTime() then target:GiveStatus("aurify", 4) end
end
