...
function ENT:DrawHUD(seat)
	...
	if self.Pilot == MySelf then
		...
		fw.y = 0
		local pos2 = (startpos + fw:GetNormalized() * 4096):ToScreen()
	...
	if self.Pilot:IsValid() then
		draw.DrawText("Pilot: "..self.Pilot:Name(), "DefaultFontSmall", x, y, COLOR_RED, TEXT_ALIGN_CENTER)
		y = y + lh
	end
	if self.LeftGunner:IsValid() then
		draw.DrawText("L-Gunner: "..self.LeftGunner:Name(), "DefaultFontSmall", x, y, COLOR_RED, TEXT_ALIGN_CENTER)
		y = y + lh
	end
	if self.RightGunner:IsValid() then
		draw.DrawText("R-Gunner: "..self.RightGunner:Name(), "DefaultFontSmall", x, y, COLOR_RED, TEXT_ALIGN_CENTER)
		y = y + lh
	end
	if self.LeftPassenger1:IsValid() then
		draw.DrawText("L-Passenger 1: "..self.LeftPassenger1:Name(), "DefaultFontSmall", x, y, COLOR_RED, TEXT_ALIGN_CENTER)
		y = y + lh
	end
	if self.LeftPassenger2:IsValid() then
		draw.DrawText("L-Passenger 2: "..self.LeftPassenger2:Name(), "DefaultFontSmall", x, y, COLOR_RED, TEXT_ALIGN_CENTER)
		y = y + lh
	end
	if self.RightPassenger1:IsValid() then
		draw.DrawText("R-Passenger 1: "..self.RightPassenger1:Name(), "DefaultFontSmall", x, y, COLOR_RED, TEXT_ALIGN_CENTER)
		y = y + lh
	end
	if self.RightPassenger2:IsValid() then
		draw.DrawText("R-Passenger 2: "..self.RightPassenger2:Name(), "DefaultFontSmall", x, y, COLOR_RED, TEXT_ALIGN_CENTER)
		y = y + lh
	end
end
...
function ENT:Draw()
	...
	if self.NextSmoke <= CurTime() then
		...
		local col = team.GetColor(self:Team()) or Color(255,255,255)
...
function RecDSD(ent, pilot, leftgunner, rightgunner, leftpassenger1, leftpassenger2, rightpassenger1, rightpassenger2)
	...
	if ent:IsValid() then
		pilot = Entity(pilot)
		leftgunner = Entity(leftgunner)
		rightgunner = Entity(rightgunner)
		leftpassenger1 = Entity(leftpassenger1)
		leftpassenger2 = Entity(leftpassenger2)
		rightpassenger1 = Entity(rightpassenger1)
		rightpassenger2 = Entity(rightpassenger2)

		if pilot:IsValid() then
			ent.Pilot = pilot
		end
		if leftgunner:IsValid() then
			ent.LeftGunner = leftgunner
		end
		if rightgunner:IsValid() then
			ent.RightGunner = rightgunner
		end
		if leftpassenger1:IsValid() then
			ent.LeftPassenger1 = leftpassenger1
		end
		if leftpassenger2:IsValid() then
			ent.LeftPassenger2 = leftpassenger2
		end
		if rightpassenger1:IsValid() then
			ent.RightPassenger1 = rightpassenger1
		end
		if rightpassenger2:IsValid() then
