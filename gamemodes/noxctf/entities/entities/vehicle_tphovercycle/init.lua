...
function ENT:CreateChildren()
	...
	if wing:IsValid() then
		...
		wing:Spawn()
		wing:SetVehicleParent(self)
		local phys = wing:GetPhysicsObject()
		phys:EnableGravity(false)
		constraint.Weld(self, wing, 0, 0, 0, 1)
		self:DeleteOnRemove(wing)
		self.RightWing = wing
		timer.SimpleEx(0, util.SpriteTrail, wing, 0, color_white, false, 32, 8, 0.65, 0.05, "trails/plasma.vmt")
	end
	...
	if wing2:IsValid() then
		...
		wing2:Spawn()
		wing2:SetVehicleParent(self)
		local phys = wing2:GetPhysicsObject()
		phys:EnableGravity(false)
		constraint.Weld(self, wing2, 0, 0, 0, 1)
		self:DeleteOnRemove(wing2)
		self.LeftWing = wing2
		timer.SimpleEx(0, util.SpriteTrail, wing2, 0, color_white, false, 32, 8, 0.65, 0.05, "trails/plasma.vmt")
	end
	...
	if driver:IsValid() and not self:IsMDF() then
		...
		if driver:KeyDown(IN_ATTACK) and self.NextShoot <= CurTime() then
			...
			if proj:IsValid() then
				...
				local col = team.GetColor(proj:GetTeamID())
				...
				if phys:IsValid() then
					...
					phys:SetVelocityInstantaneous(dir:GetNormalized() * math.max(self:GetVelocity():Length() * 1.2, 775))
				end
			end
		end
	end
...
function ENT:Destroyed(attacker, inflictor)
	...
	if driverisvalid then
		...
		driver:TakeSpecialDamage(driver:Health() + 29, DMGTYPE_FIRE, attacker, inflictor)
	end

	ExplosiveDamage(attacker, self, 280, 280, 1, 0.5, 5)
